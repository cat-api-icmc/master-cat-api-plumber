setMethod("initialize", signature(.Object = "Design"),
          function (.Object, ...) {
  .local <- function (.Object, method, criteria, nfact, design, 
                      start_item, preCAT, nitems) 
  {
      ...
      if (person$score) {
        ...
        if (method != "fixed") {
          if (all(is.na(responses2))) {
            person$Update_thetas(person$thetas, person$thetas_SE_history[nrow(person$thetas_SE_history), 
            ])
          }
          else {
            suppressWarnings(tmp <- mirt::fscores(test@mo, 
                                                  method = method, response.pattern = responses2, 
                                                  ...
                                                  start = person$thetas))
            fact_names <- mirt::extract.mirt(test@mo, 
                                             "factorNames")
            if (all(is.finite(tmp[, fact_names, drop = FALSE]))) {
              person$Update_thetas(tmp[, fact_names, 
                                       drop = FALSE], tmp[, paste0("SE_", fact_names), 
                                                          drop = FALSE])
            }
            else {
              person$Update_thetas(person$thetas, person$thetas_SE_history[nrow(person$thetas_SE_history), 
    ...
    else if (criteria %in% c("DPrule", "TPrule", "EPrule", 
                             "WPrule", "MEPV", "APrule")) {
      .Object@criteria_estimator <- "MAP"
    }
    ...
    if (!all(is.nan(start_item)) && all(.Object@start_item != 
                                        1) && criteria == "seq" && is.null(design$customNextItem)) 
      stop("start_item must equal 1 with seq criteria", 
           call. = FALSE)
    if (nfact > 1L && !any(criteria %in% c("Drule", "Trule", 
                                           "Wrule", "KL", "KLn", "Erule", "seq", "random", "Arule", 
                                           "APrule", "DPrule", "TPrule", "EPrule", "WPrule", "custom"))) 
    ...
    .Object@sprt_alpha <- 0.05
    .Object@sprt_beta <- 0.05
    if (length(design)) {
      dnames <- names(design)
      gnames <- c("min_SEM", "thetas.start", "min_items", 
                  "max_items", "quadpts", "max_time", "theta_range", 
                  "weights", "KL_delta", "content", "content_prop", 
                  "classify", "classify_CI", "exposure", "delta_thetas", 
                  "constraints", "customNextItem", "test_properties", 
                  "person_properties", "constr_fun", "customUpdateThetas", 
                  "allow_constrain_breaks", "sprt_lower", "sprt_upper")
      ...
      if (!is.null(design$content)) {
        ...
        if (!mirt:::closeEnough(sum(design$content_prop) - 
                                1, -1e-06, 1e-06)) 
          stop("content_prop does not sum to 1", call. = FALSE)
        tmp <- design$content_prop
        tmp <- tmp[match(names(table(.Object@content)), 
                         names(tmp))]
      ...
      if (!is.null(design$classify_CI)) {
        if (design$classify_CI > 1 || design$classify_CI < 
            0) 
      ...
      if (!is.null(design$exposure)) {
        if (length(design$exposure) != nitems) 
          stop("exposure vector length not equal to number of items", 
               call. = FALSE)
        exposure_type <- ifelse(all(design$exposure <= 
                                      1 & design$exposure >= 0), "SH", "sample")
        exposure <- if (exposure_type == "SH") 
          design$exposure
        else as.integer(design$exposure)
        if (exposure_type == "sample") 
          if (!all(exposure == design$exposure & exposure >= 
                   1)) 
      ...
      if (!is.null(design$constraints)) 
        if (!all(names(design$constraints) %in% c("independent", 
                                                  "unordered", "ordered", "not_scored", "excluded"))) 
          stop("Named element in constraints list not supported", 
               call. = FALSE)
      ...
      if (!is.null(design$constraints)) {
        if (any(names(design$constraints) == "not_scored")) {
          .Object@items_not_scored <- as.integer(design$constraints$not_scored)
          design$constraints$not_scored <- NULL
        }
        nms <- names(design$constraints)
        design$constraints[nms == "unordered"] <- lapply(design$constraints[nms == 
                                                                              "unordered"], function(x) sample(x, length(x)))
        ...
        for (i in seq_len(length(start_item))) {
          if (is.nan(start_item[i])) 
            next
          pick <- sapply(design$constraints, function(x, 
                                                      si) any(x == si), si = start_item[i])
    ...
    if (.Object@use_content && criteria == "seq") 
      stop("content designs are not supported for seq criteria", 
           call. = FALSE)
    if (length(.Object@min_SEM) != 1L && length(.Object@min_SEM) != 
        nfact) 
    ...
    if (length(preCAT)) {
      if (!is.null(design$customNextItem)) 
        stop("preCAT input not supported when customNextItem function supplied", 
             call. = FALSE)
      dnames <- names(preCAT)
      gnames <- c("min_items", "max_items", "criteria", 
                  "method", "response_variance")
      if (!all(dnames %in% gnames)) 
